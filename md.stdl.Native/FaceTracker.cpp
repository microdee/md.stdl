...
{
	_landmarks = gcnew array<Vector2>(68);
}
void DetectedFace::Update(int * pResult, int ii)
{
	...
	if (HasLandmarks)
	{
		for (int i = 0; i<68; i++)
		{
			_landmarks[i] = Vector2::Vector2((float)p[6 + 2 * i], (float)p[6 + 2 * i + 1]);
		}
	}
}

Vector2 DetectedFace::Center::get() { return Vector2::Vector2(_x, _y); }
Vector2 DetectedFace::Size::get() { return Vector2::Vector2(_width, _height); }
int DetectedFace::Neighbors::get() { return _neighbors; }
int DetectedFace::Angle::get() { return _angle; }
array<Vector2>^ DetectedFace::Landmarks::get() { return _landmarks; }
